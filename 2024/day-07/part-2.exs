...
defmodule Part2 do
  ...
  def cat(a, b) do
    (Integer.digits(a) ++ Integer.digits(b)) |> Integer.undigits()
  end
  ...
  def valid({result, [this, that]}) do
    valid({result, [this + that]}) ||
      valid({result, [this * that]}) ||
      valid({result, [cat(this, that)]})
  end

  def valid({result, [this, that | others]}) do
    valid({result, [this + that | others]}) ||
      valid({result, [this * that | others]}) ||
      valid({result, [cat(this, that) | others]})
  end
...
|> Part2.total()
|> IO.inspect(label: "wow...thats some fast recursion")
